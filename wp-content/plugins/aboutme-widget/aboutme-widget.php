...
class Aboutme_Widget extends WP_Widget {
	...
	const DDEBUG_EMAIL = 'alerts@team.about.me';
...
<?php 		}?>
			...
			if ( array_key_exists( 'error', $instance ) ) {
				...
				<?php
					...
					if ( array_key_exists( 'debug_url', $instance ) && ! empty( $instance['debug_url'] ) ) {
						if ( self::AUTO_MAILING ) {
							if ( ! wp_mail(self::DDEBUG_EMAIL, 'Wordpress Widget Profile Error!!!!!!!!!', $message . ' The api url was: '. $instance['debug_url']) ) {
								$message .= __( ' Please contact support.about.me for help.', 'aboutme-widget' );
							} else { 
								$message .= __( 'Email has been sent to support', 'aboutme-widget' );
							}
						} else {
							$message .= __( ' Please contact support.about.me for help.', 'aboutme-widget' );
						}
				...
				<?php
					...
					if ( array_key_exists( 'debug_url', $instance ) && ! empty( $instance['debug_url'] ) ) {
						if ( self::AUTO_MAILING ) {
							if ( ! wp_mail(self::DDEBUG_EMAIL, 'Wordpress Widget Registration Error!!!!!!!!!', $message . ' The api url was: '. $instance['debug_url']) ) {
								$message .= __( ' Please contact support.about.me for help.', 'aboutme-widget' );
							} else { 
								_e( 'Email has been sent to support', 'aboutme-widget' );
							}
						} else {
							$message .= __( ' Please contact support.about.me for help.', 'aboutme-widget' );
						}
				...
				<?php
					...
					<span style="font-size:80%;color:red"><?php _e( 'about.me server returns empty data. Please contact support.about.me for help.', 'aboutme-widget' ) ?></span>
				<?php
				} else if ( self::ERROR_UNKNOWN == $instance['error'] ) { ?>
					<span style="font-size:80%;color:red"><?php _e( 'An unknown error occurs while communicating with the about.me server. Please contact support.about.me for help.', 'aboutme-widget' ) ?></span>
				<?php
				}
			} ?>
			</p>
